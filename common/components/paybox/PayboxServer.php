...
{
    private function validateSignature(array $request): bool
    {
        $sign = $request['pg_sig'] ?? '';
        unset($request['pg_sig']);
        $signedRequest = ComponentContainer::getPayboxApi()->signParams($request);
    ...
    public function handle(Request $request): Response
    {
        ...
        if (!$request->isPost) {
            $response->data = ComponentContainer::getPayboxApi()->signParams(['pg_status' => 'error', 'pg_description' => 'Request is not POST']);
        ...
        if (!$requestData
            ...
            $response->data = ComponentContainer::getPayboxApi()->signParams(['pg_status' => 'error', 'pg_description' => 'Required request params missed']);

            return $response;
        }

        if (!$this->validateSignature($requestData)) {
            $response->data = ComponentContainer::getPayboxApi()->signParams(['pg_status' => 'error', 'pg_description' => 'Invalid signature']);

            return $response;
        }

        if (0 === (int) $requestData['pg_result']) {
            $response->data = ComponentContainer::getPayboxApi()->signParams(['pg_status' => 'ok', 'pg_description' => 'Payment was not done']);
        ...
        try {
            switch ($this->getTypeById($requestData['pg_order_id'])) {
                case Contract::class:
                    ...
                    $response->data = ComponentContainer::getPayboxApi()->signParams(['pg_status' => 'ok']);
            ...
            if (1 === (int) $requestData['pg_can_reject']) {
                $response->data = ComponentContainer::getPayboxApi()->signParams(['pg_status' => 'rejected', 'pg_description' => 'Internal server error']);
            } else {
                $response->data = ComponentContainer::getPayboxApi()->signParams(['pg_status' => 'ok']);
            }
        ...
        $response->data = ComponentContainer::getPayboxApi()->signParams(['pg_status' => 'error', 'pg_description' => 'Unknown error']);
